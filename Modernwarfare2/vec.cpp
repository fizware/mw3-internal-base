...
float  vec3_t::Distance(const vec3_t& other)
{
	vec3_t delta;

	return sqrt(
		(delta.x - other.x) * (delta.x - other.x) +
		(delta.y - other.y) * (delta.y - other.y) +
		(delta.z - other.z) * (delta.z - other.z));
}
...
float vec3_t::dot(const vec3_t& in) {
	return (x * in.x + y * in.y + z * in.z);
}

float vec3_t::dot(const float* in) const
{
