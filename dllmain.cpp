...
namespace imgui
{
	...
	ImFont* start(IDXGISwapChain3* pSwapChain, ID3D12CommandQueue* pCommandQueue, type::tImguiStyle SetStyleFunction)
	{
		...
		io.Fonts->AddFontFromMemoryTTF(const_cast<unsigned char*>(CProjectFont), sizeof(CProjectFont), 19.5, &CustomFont);
		io.Fonts->AddFontFromMemoryCompressedTTF(font_awesome_data, font_awesome_size, 22.5f, &icons_config, icons_ranges);
